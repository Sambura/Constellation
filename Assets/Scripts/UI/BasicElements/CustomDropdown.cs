...
/// </remarks>
public class CustomDropdown : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler
{
    ...
    private List<Canvas> _poolList = new List<Canvas>();
    ...
    /// <summary>
    /// Probably is the created dropdown
    /// </summary>
    ...
    public void Show()
    {
        ...
        // Get root Canvas.
        gameObject.GetComponentsInParent(false, _poolList);
        if (_poolList.Count == 0)
            return;

        Canvas rootCanvas = _poolList[_poolList.Count - 1];
        for (int i = 0; i < _poolList.Count; i++)
        {
            if (_poolList[i].isRootCanvas)
            {
                rootCanvas = _poolList[i];
                break;
            }
        }
